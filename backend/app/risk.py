...
async def risk_flags(db: AsyncSession) -> list[dict]:
    flags: list[dict] = []
    today = date.today()
    cutoff = today - timedelta(days=365)
    pillar_two_rows = (await db.scalars(select(PillarTwoStatus).join(PillarTwoStatus.entity))).all()
    ...
    substance_entities = (await db.scalars(select(Entity).where(Entity.entity_type.in_(["holdco","ip_co"])))).all()
    for entity in substance_entities:
        records = (await db.scalars(select(SubstanceRecord).options(selectinload(SubstanceRecord.board_meetings)).where(SubstanceRecord.entity_id==entity.id,SubstanceRecord.period>=cutoff,SubstanceRecord.period<=today))).all()
